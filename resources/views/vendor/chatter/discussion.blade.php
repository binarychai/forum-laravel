...
<div id="chatter" class="discussion">
	...
	<div id="chatter_header" style="background-color:{{ $discussion->color }}">
		<div class="container">
			<a class="back_btn" href="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}"><i class="chatter-back"></i></a>
			<h1>{{ $discussion->title }}</h1><span class="chatter_head_details"> @lang('chatter::messages.discussion.head_details')<a class="chatter_cat" href="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
            ...
            @else
                <div class="col-md-3 left-column">
                    ...
                    <div class="chatter_sidebar">
                        ...
                        <a href="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}"><i class="chatter-bubble"></i> @lang('chatter::messages.discussion.all')</a>
                        <ul class="nav nav-pills nav-stacked">
                            <?php $categories = Webdevmatics\Chatter\Models\Models::category()->all(); ?>
                            @foreach($categories as $category)
                                <li><a href="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $category->slug }}"><div class="chatter-box" style="background-color:{{ $category->color }}"></div> {{ $category->name }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                    <!-- END SIDEBAR -->
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
			            ...
			            <div id="new_discussion">
				            ...
				            <form id="chatter_form_editor" action="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/posts" method="POST">
    ...
    @if(Config::get('chatter.sidebar_in_discussion_view'))
        <div id="new_discussion_in_discussion_view">
            ...
            <form id="chatter_form_editor_in_discussion_view" action="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
                <div class="row">
                    <div class="col-md-7">
                        <!-- TITLE -->
                ...
                <div id="new_discussion_footer">
                    ...
                    <a href="{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}" class="btn btn-default pull-right" id="cancel_discussion">Cancel</a>
...
<script>
	$('document').ready(function(){
		...
		$('.discussions li').on('click', '.update_chatter_edit', function(e){
			...
			$.form('{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/posts/' + post_id, { _token: '{{ csrf_token() }}', _method: 'PATCH', 'body' : update_body }, 'POST').submit();
		});
		...
		$('.delete_response').click(function(){
			...
			$.form('{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/posts/' + post_id, { _token: '{{ csrf_token() }}', _method: 'DELETE'}, 'POST').submit();
		});
        ...
        @if(Config::get('chatter.sidebar_in_discussion_view'))
            ...
            $('#new_discussion_btn').click(function(){
                @if(Auth::guest())
                    window.location.href = "{{ config('app.url') }}/{{ Config::get('chatter.routes.home') }}/login";
                @else
